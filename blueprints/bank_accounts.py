...
class MapBankAccounts(object):
    ...
    def get_bank_accounts(self, **kwargs):
        ...
        if len(look_for) > 0:
            try:
                ...
                raise cherrypy.HTTPError(
                    400,
                    'Incorrect format/value for `filters`'
                )
                ...
                if filter['filter_by'] == 'associated_with':
                    for type in filter['value']:
                        look_for_criterias.append({
                            'associated_id': HelperAccountRelation.get_subquery_sql(
                                type=type,
                                look_for=look_for
                            ),
                            'op': 'in'
        ...
        for item in result['results']:
            ...
            item['relation'] = HelperAccountRelation.get(
                type=item['associated_with'],
                id=item['associated_id'],
                as_dict=True,
                conn=conn
            )
    ...
    def get_bank_account_by_id(self, **kwargs):
        ...
        bank_account['relation'] = HelperAccountRelation.get(
            type=bank_account['associated_with'],
            id=bank_account['associated_id'],
            as_dict=True,
            conn=conn
        )
    ...
    def patch_bank_account(self, **kwargs):
        ...
        try:
            ...
            event.code = Serie.generate(
                reference="bank-account-transactions",
    ...
    def get_bank_account_transfers(self, **kwargs):
        ...
        for item in result['results']:
            ...
            item['from_account'] = (
                BankAccount()
                .where(
                    {'bank_account_id': item['from_account_id']}
                )
                .one_or_none(
                    conn=conn
                )
            ...
            item['to_account'] = (
                BankAccount()
                .where(
                    {'bank_account_id': item['to_account_id']}
                )
                .one_or_none(
                    conn=conn
                )
    ...
    def save_bank_account_transfer(self, **kwargs):
        ...
        from_account = (
            BankAccount()
            .where(
                {'bank_account_id': body.get('from_account_id')}
            )
            .one_or_none(
                conn=conn
            )
        ...
        to_account = (
            BankAccount()
            .where(
                {'bank_account_id': body.get('to_account_id')}
            )
            .one_or_none(
                conn=conn
            )
        ...
        try:
            ...
            transfer.code = Serie.generate(
                reference="bank-account-transfers",
                key='transfers',
                zfill=6,
                conn=conn
            )
            ...
            event.code = Serie.generate(
                reference="bank-account-transactions",
                key=from_account.bank_account_id,
                zfill=6,
                conn=conn
            )
            ...
            event.code = Serie.generate(
                reference="bank-account-transactions",
                key=to_account.bank_account_id,
                zfill=6,
                conn=conn
            )
        ...
        try:
            ...
            for account in accounts:
                ...
                if balance != account['balance']:
                    logging.info(
                        'Final balance {}, current balance {}'.format(
                            balance,
                            account['balance']
                        )
                    )
            ...
            logging.error('DataBase Error: {}'.format(
                str(e)
            ))
